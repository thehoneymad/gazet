...
impl GridStore {
    ...
    pub fn new<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        Self::new_with_options(
            ...
            DEFAULT_ZOOM,
            0,
            DEFAULT_COALESCE_RADIUS,
            vec![[0, 0, max as u16, max as u16]],
            0.0,
        )
    }
...
mod tests {
    ...
    fn test_proximity_ordering() {
        ...
        let store =
            GridStore::new_with_options(dir.path(), 14, 0, 0.0, global_bbox_for_zoom(14), 0.0)
                .unwrap();
    ...
    fn test_language_penalty_outside_radius() {
        ...
        builder.finish().unwrap();
        let store =
            GridStore::new_with_options(dir.path(), 14, 0, 1.0, global_bbox_for_zoom(14), 0.0)
                .unwrap(); // Small radius
        ...
        assert_eq!(
            results[0].grid_entry.relev, 0.96,
            "Should have 4% language penalty"
        );
    }
    ...
    fn test_language_no_penalty_within_radius() {
        ...
        builder.finish().unwrap();
        let store =
            GridStore::new_with_options(dir.path(), 14, 0, 100.0, global_bbox_for_zoom(14), 0.0)
                .unwrap(); // Large radius
    ...
    fn test_morton_collision_different_phrases() {
        ...
        let results1: Vec<MatchEntry> = store
            .get_matching(&match_key1, &match_opts, 10)
            .unwrap()
            .collect();
        ...
        let match_key2 = MatchKey {
            match_phrase: MatchPhrase::Exact(2),
            lang_set: 0,
        };
        let results2: Vec<MatchEntry> = store
            .get_matching(&match_key2, &match_opts, 10)
            .unwrap()
            .collect();
